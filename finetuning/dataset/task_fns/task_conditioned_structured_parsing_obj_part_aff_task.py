...
from .task_prompts.task_conditioned_structured_parsing_intent_depth_v1 import JSON_ONLY_PREFIX, SCENE_CENTRIC_PROMPTS


class TaskConditionedStructuredParsingObjPartAffTaskFn(object):
    """JSON task fn for strict hierarchy: object -> parts -> affordances.

    - Object-centric(full): 80-90%
    - Scene-centric: 10-20%
    """
    ...
    def sanitize_object(self, obj, ori_width, ori_height):
        ...
        for part in obj.get("parts", []) or []:
            ...
            if not part_name or part_bbox is None:
                continue
            ...
            for aff in part.get("affordances", []) or []:
                ...
                point = self.normalize_point(aff.get("point"), ori_width, ori_height)
