...
class TaskConditionedStructuredParsingTSVDataset(Dataset):
    """Hierarchical TSV Dataset for task-conditioned structured parsing.

    The annotation schema is expected to be:
        {
            "objects": [
                {
                    "name": str,
                    "bbox": [x0, y0, x1, y1] or None,
                    "parts": [{"part_name": str, "bbox": [...] or None}],
                    "affordances": [{"action": str, "point": [x, y] or None}],
                }
            ]
        }

    This class is additive and does not affect the existing GroundingTSVDataset
    or PointingTSVDataset.
    """
    ...
    def __init__(
        ...
        super(TaskConditionedStructuredParsingTSVDataset, self).__init__()
        self.data = []
        f = open(ann_lineidx_file)
        for line in f:
            self.data.append(int(line.strip()))
    ...
    def convert_raw_data(self, image_pil, data_dict):
        ...
        for obj in data_dict.get("objects", []):
            ...
            for part in obj.get("parts", []):
                part_affordances = []
                for affordance in part.get("affordances", []) or []:
                    part_affordances.append(
                        {
                            "action": affordance.get("action"),
                            "point": affordance.get("point"),
                        }
                    )
                parts.append(
                    {
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if "objects" in data_dict and len(data_dict["objects"]) == 0:
            print(f"no objects in {self.dataset_name} id: {i}.")
            return self.__getitem__((i + 1) % len(self.data))
...
class PositiveBiasedTaskConditionedStructuredParsingTSVDataset(TaskConditionedStructuredParsingTSVDataset):
    """Task-conditioned TSV dataset with image-level positive-biased resampling.

    Images are bucketed by whether they contain any valid part / affordance
    supervision, then their indices are repeated according to user-provided
    weights. This keeps the Trainer path unchanged while exposing a clean
    ablation interface in config / bash.
    """

    def __init__(
        self,
        *args,
        weight_no_part_no_aff=1.0,
        weight_part_only=2.0,
        weight_aff_only=2.0,
        weight_part_and_aff=3.0,
        sampling_seed=42,
        bucket_filter="all",
        **kwargs,
    ):
        super().__init__(*args, **kwargs)
        self.weight_no_part_no_aff = float(weight_no_part_no_aff)
        self.weight_part_only = float(weight_part_only)
        self.weight_aff_only = float(weight_aff_only)
        self.weight_part_and_aff = float(weight_part_and_aff)
        self.sampling_seed = int(sampling_seed)
        self.bucket_filter = str(bucket_filter)
        self._build_positive_biased_index()

    def _get_bucket_weight(self, has_part, has_aff):
        if has_part and has_aff:
            return self.weight_part_and_aff
        if has_part:
            return self.weight_part_only
        if has_aff:
            return self.weight_aff_only
        return self.weight_no_part_no_aff

    def _bucket_allowed(self, has_part, has_aff):
        if self.bucket_filter == "all":
            return True
        if self.bucket_filter == "part_and_aff_only":
            return has_part and has_aff
        raise ValueError(f"Unsupported bucket_filter: {self.bucket_filter}")

    def _analyze_annotation_bucket(self, data_dict):
        has_part = False
        has_aff = False
        for obj in data_dict.get("objects", []):
            for part in obj.get("parts", []) or []:
                if part.get("part_name") and part.get("bbox") is not None:
                    has_part = True
                    break
            for aff in obj.get("affordances", []) or []:
                if aff.get("action") and aff.get("point") is not None:
                    has_aff = True
                    break
            if has_part and has_aff:
                break
        return has_part, has_aff

    def _build_positive_biased_index(self):
        rng = np.random.RandomState(self.sampling_seed)
        expanded = []
        ann_handle = open(self.ann_tsv_file)
        for ann_line_idx in self.data:
            ann_handle.seek(ann_line_idx)
            parts = ann_handle.readline().strip().split("	", 1)
            if len(parts) != 2:
                continue
            _, ann = parts
            data_dict = json.loads(ann)
            has_part, has_aff = self._analyze_annotation_bucket(data_dict)
            if not self._bucket_allowed(has_part, has_aff):
                continue
            weight = self._get_bucket_weight(has_part, has_aff)
            base = int(np.floor(weight))
            frac = weight - base
            repeat = max(base, 0)
            if frac > 0 and rng.rand() < frac:
                repeat += 1
            if repeat <= 0:
                continue
            expanded.extend([ann_line_idx] * repeat)
        ann_handle.close()
        if expanded:
